
import 'dart:ui';

import 'package:flutter/material.dart';

class AppColors{
   static Color activeButton=Colors.amber.withOpacity(0.7);
   static Color inActiveButton=Colors.grey.withOpacity(0.3);
  static const Color textColor1= Color(0xFF989acd);
  static const Color textColor2= Color(0xFF878593);
  static const Color boldColorText= Color(0xFF2e2e31);
  static const Color mainColor= Color(0xFF5d69b3);
  static const Color starColor= Color(0xFFe7bb4e);
  static const Color mainTextColor= Color(0xFFababad);
  static const Color buttonColor= Color(0xFFf1f2ef);

}